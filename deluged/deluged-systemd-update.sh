...
sudo sed -i "s@Group=ReplaceMe@Group=$UNAME@g" /etc/systemd/system/$APPSYSTEMD  || { echo -e $RED'Modifying GROUP in SYSTEMD file failed.'$ENDCOLOR; exit 1; }
...
sudo sed -i "s@Group=ReplaceMe@Group=$UNAME@g" /etc/systemd/system/$APPSYSTEMD2  || { echo -e $RED'Modifying GROUP in SYSTEMD2 file failed.'$ENDCOLOR; exit 1; }
