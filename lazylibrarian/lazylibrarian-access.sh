#!/bin/bash
# Script Name: AtoMiC Lazy Librarian Access Details
# Author: TommyE123
...
if grep -q 'http_host = localhost' $APPSETTINGS; then 
  echo -e $RED'WARNING: '$ENDCOLOR'You can only access lazylibrarian on localhost.'
else
  echo -e $GREEN'GOOD: '$ENDCOLOR'lazylibrarian is accessible outside localhost. Run access fix to restrict to localhost.'
fi
